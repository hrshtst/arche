...
;; This file is the primary configuration of Emacs which is loaded
;; from init.el. Since this is forked from Radon Rosborough's
;; configuration <https://github.com/raxod502/radian/emacs/radian.el>,
;; a large part of the code is stolen from his original
;; implementation, substituting the prefix "radian-" with "arche-",
;; and some part is added or modified based on my personal preference.
;; See commentary in init.el in this directory.
...
(defgroup arche-hooks nil
  ...
  :group 'arche
  :link '(url-link :tag "GitHub" "https://github.com/hrshtst/arche"))
...
(defmacro arche--with-local-load-history (&rest body)
  "Evaluate BODY as part of `arche-local-init-file'.
...
loaded from."
  ...
  `(let ((current-load-list nil))
     ,@body
     (push (cons ',arche-local-init-file current-load-list) load-history)))
  ...
  (if byte-compile-current-file
      (let ((forms nil))
        ...
        (dolist (link arche--hook-contents)
          (setf (cdr link)
                (nreverse (cdr link))))
        `(arche--with-local-load-history ,@forms))
...
Only do this once, unless AGAIN is non-nil."
  ...
  ;; concurrent (yet).
  (unless (or (not arche-env-setup)
              (and arche--env-setup-p (not again)))
...
(use-package consult
  ...
  :bind (([remap switch-to-buffer] . #'consult-buffer)
         ...
         ([remap pop-global-mark] . #'consult-mark)
...
(use-package migemo
  ...
  ;; Integration with `prescient'.
  (use-feature prescient
    :config

    (cl-defun prescient-migemo-regexp (query &key with-group
                                             &allow-other-keys)
      "Return a regexp matching QUERY as Japanese characters.
The regexp is generated by passing QUERY literally to the command
`cmigemo'.
...
If WITH-GROUP is `all', enclose the match in a capture group."
    ...
    ;; Add option for migemo to filter method choices.
    (let ((choices (get 'prescient-filter-method 'custom-type))
          (docstring
           (documentation-property
            'prescient-filter-method 'variable-documentation))
          (pos 0))
      (put 'prescient-filter-method 'custom-type
           (append '(const :tag "Migemo" migemo) choices))
      (save-match-data
        (while (string-match "Value ‘\\w+’ \\(?:.*\n\\)+?\n"
                             docstring pos)
          (setq pos (match-end 0))))
      (put 'prescient-filter-method 'variable-documentation
           (format "%sValue ‘migemo’ means the subquery is converted into a regexp
that matches Japanese characters.\n\n%s"
                   (substring docstring 0 pos)
                   (substring docstring pos)))
...
(use-feature warnings
  ...
  (arche-when-compiletime (version<= "31" emacs-version)
...
(use-feature autorevert
  ...
  (arche-defadvice arche--autorevert-quietly (func &rest args)
  ...
  (defun arche-autorevert-inhibit-polling-p (buffer)
  ...
  (arche-defadvice arche--autorevert-poll-only-visible (bufs)
    :filter-return #'auto-revert--polled-buffers
    "Inhibit `autorevert' for buffers not displayed in any window."
    (cl-remove-if #'arche-autorevert-inhibit-polling-p bufs))
...
menu to disappear and then come back after `company-idle-delay'."
  ...
  (arche-defadvice arche--advice-company-no-remote ()
...
(use-package treesit-auto
  ...
  (setq treesit-auto-langs '(bash python rust go toml yaml))
...
(use-feature flymake
  ...
  (arche-defadvice arche--flymake-inhibit (func &optional _)
    :around #'flymake-mode
...
(use-feature sh-script
  ...
  (arche-defhook arche--sh-prettify-mode-line (&rest _)
    sh-mode-hook
  ...
  (advice-add 'sh-set-shell :after #'arche--sh-prettify-mode-line))
...
;; Package `git-modes' provides major modes for various Git
;; configuration files, which include .gitattributes, .gitconfig,
...
(use-package pkgbuild-mode
  ...
  (arche-defhook arche--pkgbuild-disable-hooks ()
    pkgbuild-mode-hook
...
(use-feature oc
  ...
  (when (fboundp 'citar-insert-citation)
...
(use-package citar-denote
  ...
  :bind (("C-c n l c" . #'citar-create-note)
         ...
         ("C-c n l a" . #'citar-denote-add-reference)
         ("C-c n l k" . #'citar-denote-remove-reference)
...
(use-feature emacsql-sqlite-common
  ...
  (arche-defadvice arche--advice-emacsql-no-error-swallow ()
...
;; time to refactor `arche-new-emacs' to work based on it.
...
;; Local Variables:
;; byte-compile-warnings: (not docstrings make-local noruntime unresolved)
